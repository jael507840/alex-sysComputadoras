...
<template>
  <div v-if="isVisible" class="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/70 px-4">
    <div class="w-full max-w-4xl rounded-[2rem] border border-cyan-100 bg-gradient-to-br from-white via-slate-50 to-cyan-50 p-6 shadow-[0_20px_70px_-20px_rgba(6,182,212,0.4)] lg:p-8">
      <div class="flex justify-end">
        <button class="rounded-full bg-slate-900 px-3 py-2 text-sm font-semibold text-white transition hover:bg-cyan-600" @click="emit('close')">✕</button>
      </div>
      <div v-if="props.product" class="mt-4 grid gap-8 lg:grid-cols-[1fr_0.9fr]">
        ...
        <div>
          ...
          <div class="mt-6 flex items-center justify-between rounded-2xl bg-slate-900/95 p-4 text-white">
            <div>
              <p class="text-sm text-slate-400">Precio</p>
              <p class="text-2xl font-bold text-cyan-400">Bs. {{ props.product.price }}</p>
            </div>
            <div class="text-right">
              <p class="text-sm text-slate-400">Stock</p>
              <p class="font-semibold text-white">{{ props.product.stock }} disponibles</p>
            </div>
          </div>
          <div class="mt-6 flex flex-wrap gap-3">
            <button class="rounded-full bg-gradient-to-r from-cyan-600 to-indigo-600 px-5 py-3 font-semibold text-white transition hover:opacity-90" @click="emit('buy', props.product)">Comprar ahora</button>
